...
class sensorsHandler(webapp.RequestHandler):
	...
	def get(self):
		...
		if data_query is not None:
			...
			for temp in data_query:
				self.response.write('<blockquote>ACcurrent1 = %s </blockquote>' % cgi.escape( str(temp.ac_current1) ) )
				self.response.write('<blockquote>ACcurrent2 = %s </blockquote>' % cgi.escape( str(temp.ac_current2) ) )
				self.response.write('<blockquote>ACvoltage1 = %s </blockquote>' % cgi.escape( str(temp.ac_voltage2) ) )
				self.response.write('<blockquote>ACvoltage2 = %s </blockquote>' % cgi.escape( str(temp.ac_voltage2) ) )

				self.response.write('<blockquote>DCcurrent1 = %s </blockquote>' % cgi.escape( str(temp.dc_current1) ) )
				self.response.write('<blockquote>DCcurrent2 = %s </blockquote>' % cgi.escape( str(temp.dc_current2) ) )
				self.response.write('<blockquote>DCcurrent3 = %s </blockquote>' % cgi.escape( str(temp.dc_current3) ) )
				self.response.write('<blockquote>DCcurrent4 = %s </blockquote>' % cgi.escape( str(temp.dc_current4) ) )

				self.response.out.write('<blockquote>DCvoltage1 = %s </blockquote>' % cgi.escape( str(temp.dc_voltage1) ) )
				self.response.out.write('<blockquote>DCvoltage1 = %s </blockquote>' % cgi.escape( str(temp.dc_voltage2) ) )
				self.response.out.write('<blockquote>DCvoltage1 = %s </blockquote>' % cgi.escape( str(temp.dc_voltage3) ) )
				self.response.out.write('<blockquote>DCvoltage1 = %s </blockquote>' % cgi.escape( str(temp.dc_voltage4) ) )
